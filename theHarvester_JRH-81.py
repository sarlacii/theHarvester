...
if sys.version_info[0] >= 3:
    print("Error! You must run this script with Python 2!")
    sys.exit()
else:
    print("Python version is %s" % str(sys.version_info[:2]))
...
try:
    ...
    print("Python2 requests library not found, please install it before proceeding!\n")
    sys.exit()
            ...
            try:
                ...
                if not shodanvisited.count(ip):
                    ...
                    for res in results['matches']:
                        #print(str("%s:%s - %s" % (res['ip_str'], res['port'], res['product'])))
                    #    shodanres.append(x + "SAPO" + str(res['product']) + "SAPO" + str(res['port']))
